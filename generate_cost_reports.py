...
"""
generate_cost_reports.py - Skrypt do generowania raportu rzeczywistych kosztów produkcji w XLSX.

Instrukcje użycia:
- Uruchamiany z argumentem ścieżki do folderu Raporty.
- Odczytuje data.json, oblicza realne koszty bez narzutów dynamicznych.
- Dodaje gięcie pomniejszone o 25%, dodatkowe.
- Wylicza koszty jednostkowe, całkowite, cenę sprzedaży z marżą.
- Zapisuje do "Raport kosztów.xlsx" i dopisuje do logu.
...
def main(raporty_path):
    ...
    with open(log_path, 'a', encoding='utf-8') as log:
        log.write("\n--- Raport rzeczywistych kosztów (bez narzutów dynamicznych) ---\n")

    # Tworzenie XLSX
    wb = Workbook()
    ws = wb.active
    ws.title = "Rzeczywiste koszty"

    # Nagłówki
    ws.append(["ID", "Nazwa", "Materiał", "Grubość", "Ilość", "Koszt jednostkowy (bez narzutów)", "Gięcie (75%)", "Dodatkowe", "Koszt całkowity", "Cena sprzedaży z marżą"])

    total_cost = 0.0
    for part in all_parts:
        unit_cost = part['base_cost_per_unit']  # Bez narzutów dynamicznych
    ...
    wb.save(os.path.join(raporty_path, "Raport kosztów.xlsx"))

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Użycie: python generate_cost_reports.py <raporty_path>")
    else:
        main(sys.argv[1])
