...
tk.Label(left_frame, text="Wybierz folder:", bg="#808080").grid(row=0, column=0, sticky="e")
tk.Entry(left_frame, textvariable=folder_var, width=50).grid(row=0, column=1)
tk.Button(left_frame, text="Przeglądaj", command=select_folder).grid(row=0, column=2)

tk.Label(left_frame, text="Nazwa klienta:", bg="#808080").grid(row=1, column=0, sticky="e")
tk.Entry(left_frame, textvariable=customer_var).grid(row=1, column=1)
tk.Label(left_frame, text="Numer oferty:", bg="#808080").grid(row=2, column=0, sticky="e")
tk.Entry(left_frame, textvariable=offer_var).grid(row=2, column=1)
tk.Label(left_frame, text="Data oferty:", bg="#808080").grid(row=3, column=0, sticky="e")
tk.Entry(left_frame, textvariable=date_var).grid(row=3, column=1)
tk.Label(left_frame, text="Okres ważności:", bg="#808080").grid(row=4, column=0, sticky="e")
tk.Entry(left_frame, textvariable=validity_var).grid(row=4, column=1)

def upload_logo():
    p = filedialog.askopenfilename(filetypes=[("Pliki obrazów", "*.png;*.jpg;*.jpeg")])
    if p: logo_var.set(p)

tk.Label(left_frame, text="Wczytaj logo:", bg="#808080").grid(row=5, column=0, sticky="e")
tk.Entry(left_frame, textvariable=logo_var, width=50).grid(row=5, column=1)
tk.Button(left_frame, text="Przeglądaj", command=upload_logo).grid(row=5, column=2)

tk.Label(left_frame, text="Szczegóły kontaktowe:", bg="#808080").grid(row=6, column=0, sticky="ne")
contact_text = tk.Text(left_frame, height=5, width=50)
...
tk.Label(left_frame, text="Tekst poprzedzający:", bg="#808080").grid(row=7, column=0, sticky="ne")
preceding_text_var = tk.Text(left_frame, height=5, width=50)
...
tk.Label(left_frame, text="", bg="#808080").grid(row=8, column=0, pady=10)
tk.Label(left_frame, text="Tekst kończący:", bg="#808080").grid(row=9, column=0, sticky="ne")
finishing_text_var = tk.Text(left_frame, height=10, width=50)
finishing_text_var.grid(row=9, column=1)
finishing_text_var.insert(tk.INSERT, "Wyłączenia odpowiedzialności ...")

tk.Label(left_frame, text="Odczytane pliki:", bg="#808080").grid(row=10, column=0, sticky="ne")
file_list = tk.Listbox(left_frame, height=5, width=50)
...
buttons_frame = tk.Frame(left_frame, bg="#808080")
...
right_paned = tk.PanedWindow(right_frame, orient=tk.VERTICAL, bg="#808080", sashrelief="raised", borderwidth=1)
right_paned.pack(fill="both", expand=True)

panel_a = tk.PanedWindow(right_paned, orient=tk.VERTICAL, bg="#808080", sashrelief="raised", borderwidth=1)

# --- PANEL 1 ---
subpanel1 = tk.LabelFrame(panel_a, text="PANEL 1 — PODGLĄD", bg="#808080", fg="white")
columns = ("1", "2", "3", "4", "5", "6", "7", "8", "9")
tree = ttk.Treeview(subpanel1, columns=columns, show="headings")
tree.heading("#0", text="Miniatura")
tree.column("#0", width=90, minwidth=70, stretch=tk.NO)
...
# --- kluczowa zmiana: minsize, aby panel 1 był zawsze widoczny ---
panel_a.add(subpanel1, minsize=220)

# --- PANEL 2 ---
subpanel2 = tk.LabelFrame(panel_a, text="PANEL 2 — STAŁE KOSZTY", bg="#808080", fg="white")
tk.Label(subpanel2, text="Koszty operacyjne za arkusz:", bg="#808080", fg="white").grid(row=0, column=0, sticky="w")
op_cost_entry = tk.Entry(subpanel2); op_cost_entry.grid(row=0, column=1); op_cost_entry.insert(tk.INSERT, "40,00")
tk.Label(subpanel2, text="Technologia za zlecenie:", bg="#808080", fg="white").grid(row=1, column=0, sticky="w")
tech_order_entry = tk.Entry(subpanel2); tech_order_entry.grid(row=1, column=1); tech_order_entry.insert(tk.INSERT, "50,00")
tk.Label(subpanel2, text="Dodatkowe koszty dla zlecenia (np. narzędzia):", bg="#808080", fg="white").grid(row=2, column=0, sticky="w")
add_order_cost_entry = tk.Entry(subpanel2); add_order_cost_entry.grid(row=2, column=1); add_order_cost_entry.insert(tk.INSERT, "0,00")
panel_a.add(subpanel2, minsize=120)

# --- PANEL 3 ---
subpanel3 = tk.LabelFrame(panel_a, text="PANEL 3 — CENNIKI I TESTY", bg="#808080", fg="white", padx=6, pady=6)

mat_frame = tk.LabelFrame(subpanel3, text="Cennik materiałów (PLN/kg)", bg="#808080", fg="white")
...
btn_load_mat = tk.Button(mat_frame, text="Załaduj cennik materiałów", command=lambda: load_material_prices(preview=True))
btn_load_mat.grid(row=0, column=0, sticky="w")
material_led = tk.Canvas(mat_frame, width=20, height=20, bg="#808080", highlightthickness=0); material_led.grid(row=0, column=1, padx=(6,0)); _update_led(material_led, False)
tk.Label(mat_frame, text="Materiał:", bg="#808080", fg="white").grid(row=1, column=0, sticky="e", pady=(6,0))
...
tk.Label(mat_frame, text="Grubość [mm]:", bg="#808080", fg="white").grid(row=2, column=0, sticky="e")
...
btn_find_mat = tk.Button(mat_frame, text="Znajdź cenę materiału", command=lambda: ui_find_material_price()); btn_find_mat.grid(row=3, column=0, columnspan=2, pady=4, sticky="we")
material_result_label = tk.Label(mat_frame, text="Wynik Materiał: —", bg="#808080", fg="white"); material_result_label.grid(row=4, column=0, columnspan=2, sticky="w")

cut_frame = tk.LabelFrame(subpanel3, text="Cennik cięcia (PLN/m)", bg="#808080", fg="white")
cut_frame.grid(row=0, column=1, sticky="nwe", padx=4, pady=4)
btn_load_cut = tk.Button(cut_frame, text="Załaduj cennik cięcia", command=lambda: load_cutting_prices(preview=True))
btn_load_cut.grid(row=0, column=0, sticky="w")
cutting_led = tk.Canvas(cut_frame, width=20, height=20, bg="#808080", highlightthickness=0); cutting_led.grid(row=0, column=1, padx=(6,0)); _update_led(cutting_led, False)
tk.Label(cut_frame, text="Materiał:", bg="#808080", fg="white").grid(row=1, column=0, sticky="e", pady=(6,0))
...
tk.Label(cut_frame, text="Grubość [mm]:", bg="#808080", fg="white").grid(row=2, column=0, sticky="e")
...
tk.Label(cut_frame, text="Gaz:", bg="#808080", fg="white").grid(row=3, column=0, sticky="e")
...
btn_find_cut = tk.Button(cut_frame, text="Znajdź cenę cięcia", command=lambda: ui_find_cutting_price()); btn_find_cut.grid(row=4, column=0, columnspan=2, pady=4, sticky="we")
cutting_result_label = tk.Label(cut_frame, text="Wynik Cięcie: —", bg="#808080", fg="white"); cutting_result_label.grid(row=5, column=0, columnspan=2, sticky="w")

btn_load_both = tk.Button(subpanel3, text="Załaduj oba cenniki i odśwież listy",
...
def analyze_xlsx_folder():
    ...
    for fname in files:
        ...
        try:
            ...
            ws_all_part_list = wb['All Parts List']
            loader = SheetImageLoader(ws_all_part_list)
            ...
            while row <= cost_sheet.max_row and isinstance(cost_sheet.cell(row=row, column=1).value, (int,float)):
                ...
                # Pobierz thumbnail z kolumny B
                cell = f'B{row-4}'
                thumbnail = None
                if loader.image_in(cell):
                    image = loader.get(cell)  # PIL Image
                    # Konwertuj na bajty do przechowywania w słowniku (do raportu PDF/HTML)
                    img_byte_arr = io.BytesIO()
                    image.save(img_byte_arr, format='PNG')
                    thumbnail = img_byte_arr.getvalue()  # Bajty obrazu
    ...
    for i, p in enumerate(all_parts, start=1):
        tree.insert('', 'end', values=(i, p['subnr'],p['name'], p['material'], f"{p['thickness']}", p['qty'], format_pln(p['cost_per_unit']), "", ""))
...
# przyciski lewe
tk.Button(buttons_frame, text="Analizuj XLSX", command=analyze_xlsx_folder).pack(side="left", padx=5)
tk.Button(buttons_frame, text="Generuj raport", command=generate_report).pack(side="left")
