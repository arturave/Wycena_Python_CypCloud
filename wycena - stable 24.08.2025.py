...
marza_var    = tk.StringVar(value="20")  # Nowe pole dla marży (%)
...
ttk.Label(subpanel2, text="Dodatkowe koszty dla zlecenia (np. narzędzia):").grid(row=2, column=0, sticky="w")
...
ttk.Label(subpanel2, text="Marża (%):").grid(row=3, column=0, sticky="w")
marza_entry = ttk.Entry(subpanel2, textvariable=marza_var); marza_entry.grid(row=3, column=1)
...
def analyze_xlsx_folder():
    ...
    log_pathx = os.path.join(raporty_path, "part_calculation_log.txt")
    with open(log_pathx, 'w', encoding='utf-8') as log_part:
        log_part.write(f"Log obliczeń - {datetime.datetime.now()}\n")
        log_part.write(f"Folder: {folder_path}\n")
        log_part.write(f"Klient: {customer_name}\n")
        log_part.write("Źródła cen: materials prices.xlsx, cutting prices.xlsx\n")
        log_part.write("\nSzczegóły obliczeń:\n")

    global all_parts, last_groups, last_total_cost, last_folder_path, total_sheets, total_parts_qty
    for item in tree.get_children():
        tree.delete(item)
    ...
    marza_var    = tk.StringVar(value="20")  # Nowe pole dla marży (%)
    marza = _parse_float(marza_var.get()) or 20.0
    ...
    for fname in files:
        ...
        try:
            ...
            with open(log_pathx, 'a', encoding='utf-8') as log_part:
                log_part.write(f"Analiza pliku : {path}\n");
            ...
            if util_row is None:
                with open(log_pathx, 'a', encoding='utf-8') as log_part:
                    log_part.write("Nie znaleziono 'Average utilization:'\n");
            ...
            if utilization_rate <= 0 or utilization_rate > 1:
                ...
                with open(log_pathx, 'a', encoding='utf-8') as log_part:
                    log_part.write("Uwaga", f"Average utilization poza zakresem ({utilization_rate}).\n");
            with open(log_pathx, 'a', encoding='utf-8') as log_part:
                log_part.write(f"utilization_rate: {utilization_rate}\n");
            mat_price_row = None
            for r in range(1, cost_sheet.max_row + 1):
            ...
            while row <= cost_sheet.max_row and isinstance(cost_sheet.cell(row=row, column=1).value, (int, float)):
                ...
                total_part = total_part* (1 + marza / 100)
                ...
                with open(log_pathx, 'a', encoding='utf-8') as log_part:
                    log_part.write(f"Part ID: {lp}, Name: {part_name}, Marża: {marza}, Material: {material_name}, Thickness: {thk_val}, Qty: {part_qty}, Cost per unit: {total_part:.2f}, Base cost per unit: {base_total_part:.2f}, Raw weight: {weight}, Contours qty: {contours_qty}, Cut length: {cut_length}, Marking length: {marking_length}, Defilm length: {defilm_length}, Adj weight: {adj_weight}, Base rate per cut length: {base_rate_per_cut_length}, Base cut cost: {base_cut_cost}\n")
    ...
    with open(log_pathx, 'a', encoding='utf-8') as log_part:
        log_part.write(f"extra_per_part: {extra_per_part}, op_cost_per_part: {op_cost_per_part}\n")
    ...
    with open(log_pathx, 'a', encoding='utf-8') as log_part:
        log_part.write(f"total_sum: {total_sum}\n")
...
def generate_report():
    ...
    if len(tree_items) != len(all_parts):
    ...
    hdr[3].text = 'Ilość'
    hdr[4].text = 'Koszt (PLN)'
    hdr[5].text = 'Razem (PLN)'
    ...
    for mat_name, thk, parts in last_groups:
        ...
        for nm, cost_per_unit, qty in parts:
            ...
            cost_per_unit += part['bending_per_unit'] + part['additional_per_unit']
            ...
            r[3].paragraphs[0].add_run(f"{int(qty)}  ").font.size = Pt(10)
            r[4].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
            r[4].paragraphs[0].add_run(f"{format_pln(cost_per_unit)}  ").font.size = Pt(10)
            row_total = cost_per_unit * qty
            r[5].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
            r[5].paragraphs[0].add_run(f"{format_pln(row_total)}  ").font.size = Pt(10)
            total += row_total
            lp += 1
    ...
    srow[1].text = ""
    srow[2].text = "Razem"
    srow[5].text = format_pln(total)
    srow[5].paragraphs[0].alignment = WD_ALIGN_PARAGRAPH.RIGHT
    for r in srow[2].paragraphs[0].runs:
        r.bold = True
    for r in srow[5].paragraphs[0].runs:
        r.bold = True

    widths = [Cm(1), Cm(2), Cm(6), Cm(2), Cm(3), Cm(3)]
    ...
    cost_ws.append(["ID", "Nazwa", "Materiał", "Grubość", "Ilość", "Koszt jednostkowy (bez narzutów)", "Gięcie (75%)", "Dodatkowe", "Koszt całkowity", "Cena sprzedaży z marżą", "Miniatura"])
    ...
    for part in all_parts:
        ...
        cost_ws.append([part['id'], part['name'], part['material'], part['thickness'], part['qty'], unit_cost, bending, additional, total_part, sales_price])
        ...
        if part['thumb_data']:
            ...
            img.height = 1.0
            cost_ws.add_image(img, 'K' + str(row_num))

        # Log
        with open(log_path, 'a', encoding='utf-8') as log:
            log.write(f"{part['name']}: Jednostkowy (bez narzutów) {unit_cost}, Gięcie {bending}, Dodatkowe {additional}, Całkowity {total_part}, Sprzedaż {sales_price}\n")
    ...
    client_ws.append(["ID", "Nazwa", "Materiał", "Grubość", "Ilość", "Koszt jednostkowy (z narzutami)", "Gięcie", "Dodatkowe", "Koszt całkowity", "Cena sprzedaży z marżą", "Miniatura"])
    ...
    for part in all_parts:
        ...
        client_ws.append([part['id'], part['name'], part['material'], part['thickness'], part['qty'], unit_cost, bending, additional, total_part, sales_price])
        ...
        if part['thumb_data']:
            ...
            img.width = 1.0  # Adjust size as needed
            img.height = 1.0
            client_ws.add_image(img, 'K' + str(row_num))

        # Log
        with open(log_path, 'a', encoding='utf-8') as log:
            log.write(f"{part['name']}: Jednostkowy (z narzutami) {unit_cost}, Gięcie {bending}, Dodatkowe {additional}, Całkowity {total_part}, Sprzedaż {sales_price}\n")
